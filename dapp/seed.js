...
module.exports = async function(done) {
    try {
        ...
        let values = await Promise.all([
            ...
            await PeerReviewWorkflow.deployed()
        ]);
        let factory = values[0];
        let workflow = values[1];
        ...
        let assets = txs.map(function(tx, i) {
            console.log('  Paper ' + i + ': ' + (tx.receipt.status? 'OK' : 'FAILED'));
            return tx.logs[0].args.asset;
        });
        
        console.log("Progress some papers to state: Published...");
        console.log("Reviewing last 3 papers.");
        txs = await Promise.all([
            await workflow.review(assets[6], { from: '0xcf73F5E46E282b3461A10c9908D47FE8142C2592' }),
            await workflow.review(assets[6], { from: '0x76D6c04849EF78603eB12C63629b361488ac059B' }),
            await workflow.review(assets[6], { from: '0x9e94B41797507584D3388DaFEd10957347C8a81D' }),
            await workflow.review(assets[7], { from: '0xcf73F5E46E282b3461A10c9908D47FE8142C2592' }),
            await workflow.review(assets[7], { from: '0x76D6c04849EF78603eB12C63629b361488ac059B' }),
            await workflow.review(assets[7], { from: '0x9e94B41797507584D3388DaFEd10957347C8a81D' }),
            await workflow.review(assets[8], { from: '0xcf73F5E46E282b3461A10c9908D47FE8142C2592' }),
            await workflow.review(assets[8], { from: '0x76D6c04849EF78603eB12C63629b361488ac059B' }),
            await workflow.review(assets[8], { from: '0x9e94B41797507584D3388DaFEd10957347C8a81D' })
        ]);
        txs.forEach(function(tx, i) {
            console.log('  Review ' + i + ': ' + (tx.receipt.status? 'OK' : 'FAILED'));
        });
        
        console.log("Accepting last 3 papers.");
        txs = await Promise.all([
            await workflow.accept(assets[6], 'Good job.', { from: '0xcf73F5E46E282b3461A10c9908D47FE8142C2592' }),
            await workflow.accept(assets[6], 'I could reproduce your results.', { from: '0x76D6c04849EF78603eB12C63629b361488ac059B' }),
            await workflow.accept(assets[6], 'Everything is clearly explained.', { from: '0x9e94B41797507584D3388DaFEd10957347C8a81D' }),    
            await workflow.accept(assets[7], 'Good job.', { from: '0xcf73F5E46E282b3461A10c9908D47FE8142C2592' }),
            await workflow.accept(assets[7], 'I could reproduce your results.', { from: '0x76D6c04849EF78603eB12C63629b361488ac059B' }),
            await workflow.accept(assets[7], 'Everything is clearly explained.', { from: '0x9e94B41797507584D3388DaFEd10957347C8a81D' }),
            await workflow.accept(assets[8], 'Good job.', { from: '0xcf73F5E46E282b3461A10c9908D47FE8142C2592' }),
            await workflow.accept(assets[8], 'I could reproduce your results.', { from: '0x76D6c04849EF78603eB12C63629b361488ac059B' }),
            await workflow.accept(assets[8], 'Everything is clearly explained.', { from: '0x9e94B41797507584D3388DaFEd10957347C8a81D' })
        ]);
        txs.forEach(function(tx, i) {
            console.log('  Accept ' + i + ': ' + (tx.receipt.status? 'OK' : 'FAILED'));
        });
